...
const ResultsPanel = ({
  ...
  if (isLoading) {
    return <div className="p-4 flex flex-col items-center justify-center min-h-[200px]">
  ...
  if (buses.length === 0 && !searched) {
    return <div className="p-4 bg-white rounded-lg shadow-md">
        <div className="flex flex-col items-center justify-center py-6">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1">Search for buses</h3>
          <p className="text-gray-500 text-center">
  ...
  return <div className="space-y-1">
      ...
      <div className="p-4 bg-white rounded-b-lg shadow-md max-h-[calc(100vh-450px)] overflow-y-auto">
        {buses.map(bus => <BusCard key={bus.id} bus={bus} onGetDirections={onGetDirections} />)}
      </div>
    </div>;
};
